...
const AREAS = [
  'ESTRATEGIA', 'RH', 'DP', 'JURIDICO', 'SST', 'NUTRICAO',
  'FINANCEIRO', 'COMERCIAL', 'QUALIDADE', 'MELHORIA_CONTINUA',
  'OPERACOES', 'COMPRAS', 'TI', 'AGRO'
]
...
export default function PlanoAcaoPage() {
        ...
        const mockAcoes: Acao[] = [
          {
            ...
            descricao: 'Criar programa estruturado de integração para novos colaboradores com duração de 30 dias',
            area: 'RH',
            prioridade: 'ALTA',
            prazo: '2026-09-15',
            ...
            status: 'PENDENTE',
            recursos: 'R$ 5.000',
            indicador_sucesso: 'Taxa de retenção > 90% no primeiro ano',
            impacto_imv: 15,
            created_at: new Date().toISOString()
          },
          {
            id: '2',
            titulo: 'Implementar Sistema de Gestão da Qualidade (SGQ)',
            descricao: 'Estruturar SGQ baseado na ISO 9001 e iniciar processo de certificação',
            area: 'QUALIDADE',
            prioridade: 'ALTA',
            prazo: '2026-12-30',
            ...
            status: 'EM_ANDAMENTO',
            recursos: 'R$ 30.000',
            indicador_sucesso: 'Certificação ISO 9001 em 12 meses',
            impacto_imv: 20,
            created_at: new Date().toISOString()
          },
          {
            id: '3',
            titulo: 'Revisar Cardápio com Nutricionista',
            descricao: 'Contratar nutricionista para revisar o cardápio e implementar BPF',
            area: 'NUTRICAO',
            prioridade: 'MEDIA',
            prazo: '2026-10-01',
            responsavel: 'RH / Nutricionista',
            status: 'PENDENTE',
            recursos: 'R$ 10.000',
            indicador_sucesso: 'Adequação nutricional > 85%',
            impacto_imv: 10,
            created_at: new Date().toISOString()
          }
        ]
      ...
      const novasAcoes: Acao[] = [
        {
          ...
          titulo: 'Implementar Pesquisa de Clima Organizacional',
          descricao: 'Realizar pesquisa de clima com metodologia validada e criar plano de ação baseado nos resultados',
          area: 'RH',
          prioridade: 'ALTA',
          prazo: '2026-09-30',
          responsavel: 'Gerente de RH',
          status: 'PENDENTE',
          recursos: 'R$ 3.000',
          indicador_sucesso: 'eNPS > 50 e participação > 80%',
          impacto_imv: 12,
          created_at: new Date().toISOString()
        },
        {
          id: (Date.now() + 1).toString(),
          titulo: 'Implementar Controle de Ponto Digital',
          descricao: 'Substituir controle manual por sistema digital com biometria',
          area: 'DP',
          prioridade: 'MEDIA',
          prazo: '2026-11-15',
          responsavel: 'Coordenador de DP',
          status: 'PENDENTE',
          recursos: 'R$ 8.000',
          indicador_sucesso: 'Redução de erros em DP > 90%',
          impacto_imv: 8,
          created_at: new Date().toISOString()
        }
      ]
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const novaAcao: Acao = {
        ...
        prazo: formData.prazo || new Date(Date.now() + 7776000000).toISOString().split('T')[0],
  ...
  const handleDelete = async (id: string, titulo: string) => {
    if (!confirm(`Tem certeza que deseja excluir a ação "${titulo}"?`)) {
      return
    }
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-4">
            <p className="text-xs text-[#5E6C84] font-medium">Total de Ações</p>
            <p className="text-2xl font-bold text-[#0A3D78]">{acoes.length}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <p className="text-xs text-[#5E6C84] font-medium">Prioridade Alta</p>
            <p className="text-2xl font-bold text-red-500">
              {acoes.filter(a => a.prioridade === 'ALTA').length}
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <p className="text-xs text-[#5E6C84] font-medium">Impacto IMV™ Total</p>
            <p className="text-2xl font-bold text-green-500">+{totalImpacto}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <p className="text-xs text-[#5E6C84] font-medium">Concluídas</p>
            <p className="text-2xl font-bold text-[#0F5FA8]">
              {acoes.filter(a => a.status === 'CONCLUIDO').length}
            </p>
          </CardContent>
        </Card>
      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Ex: Implementar programa de onboarding"
                    required
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id="impacto_imv"
                    type="number"
                    value={formData.impacto_imv}
                    onChange={(e) => setFormData({ ...formData, impacto_imv: parseInt(e.target.value) || 0 })}
                    min={0}
                    max={100}
                  />
                </div>
                ...
                <div className="space-y-2 md:col-span-2">
                  ...
                  <Input
                    ...
                    placeholder="Ex: R$ 5.000, 2 pessoas, 3 meses"
                  />
                </div>
                ...
                <div className="space-y-2 md:col-span-2">
                  ...
                  <Input
                    id="indicador_sucesso"
                    value={formData.indicador_sucesso}
                    onChange={(e) => setFormData({ ...formData, indicador_sucesso: e.target.value })}
                    placeholder="Ex: Redução de turnover em 20%"
                  />
                </div>
              </div>
            ...
            <p className="text-[#5E6C84]">
              ...
              {acoes.length === 0 && (
                ...
                <span className="text-sm">
                  Clique em "Gerar com IA" para criar recomendações automaticamente.
                </span>
              )}
            </p>
          </CardContent>
              ...
              <CardContent className="p-6">
                <div className="flex flex-col md:flex-row justify-between gap-4">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap items-center gap-3 mt-2 text-xs">
                          <span className="px-2 py-0.5 rounded-full bg-[#EAF3FC] text-[#0F5FA8]">
                            {AREAS_LABELS[acao.area] || acao.area}
                          </span>
                          <span className={`px-2 py-0.5 rounded-full ${STATUS_COLORS[acao.status]}`}>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <Select
                      value={acao.status}
                      onValueChange={(v) => handleStatusChange(acao.id, v as StatusAcao)}
                    >
                    ...
                    <Button
                      ...
                      onClick={() => handleDelete(acao.id, acao.titulo)}
                    >
